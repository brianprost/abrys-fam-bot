import { StackContext, Api, Cron, Function } from "sst/constructs";
import { config } from "dotenv";
config();

export function PromoteItOnAbrysFamBot({ stack }: StackContext) {
	const promoteItOnAbrysFam = new Function(stack, "promoteItOnAbrysFam", {
	...
	new Cron(stack, "Cron-Promote-It-On-Abrys-Fam", {
		schedule: "rate(10 minutes)",
		job: "packages/functions/src/promote-it.handler",
	});

	stack.addOutputs({
		PromoteItOnAbrysFamUrl: promoteItOnAbrysFam.url,
	});
}
